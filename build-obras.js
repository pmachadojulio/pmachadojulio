...
const path = require('path');
  ...
  const YOUTUBE_URL = 'https://www.youtube.com/@Jcmachadoart';
...
function youtubeLink(o) {
  ...
  if (!url) return '';
  return `<a href="${escapeHtml(url)}" class="btn-yt" target="_blank" rel="noopener">Ver el proceso de esta obra en YouTube</a>`;
}

function procesoHTML(o) {
  const t = youtubeLink(o);
  if (!t) return '';
...
const SHARED_STYLE = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn-yt { display: flex; align-items: center; justify-content: center; gap: 8px; background: #111; color: #fff; padding: 12px 20px; border-radius: 2px; font-family: var(--sans); font-size: 11px; font-weight: 500; letter-spacing: 1px; text-transform: uppercase; text-decoration: none; transition: background 0.2s; }
  .btn-yt:hover { background: #2a2a2a; }
